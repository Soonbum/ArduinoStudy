...
* 전자 회로를 설계하고 실제 하드웨어 제품으로 만드는 데 필수적인 EDA(Electronic Design Automation) 툴이 있음
  | 특징 | KiCad | EasyEDA |
  | ---- | ----- | ------- |
  | 운영 방식 | PC 설치형 (오픈소스) | 웹 브라우저 기반 (클라우드) |
  | 가격 | 완전 무료 (기부 형태) | 무료 (일부 고급 기능 유료 가능) |
  | 학습 난이도 | 다소 높음 (전문적 설정 필요) | 매우 낮음 (입문자 친화적) |
  | 라이브러리 | 사용자가 직접 관리/추가  | LCSC(부품몰)와 연동되어 매우 방대함 |
  | 제작 연동 | Gerber 파일을 추출하여 업체 전달 | 클릭 몇 번으로 JLCPCB에 주문 가능 |
